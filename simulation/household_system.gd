...
static func calculate(household: Dictionary, actors: Dictionary, economy: Dictionary,
		food_index: int, earned_by_actor: Dictionary, participation: Dictionary = {},
		external_income: int = 0) -> Dictionary:
	var income: int = 0
	var food: int = 0
	...
	for id: String in household.member_ids:
		...
		if fraction > 0:
			...
			food += int(base_food * food_index * fraction / 1000000.0)
			essentials += int(int(economy.essentials_per_person) * fraction / 1000.0)
	var rent: int = int(economy.rent) if living_members > 0 else 0
	var available_income: int = income + external_income
	...
	var savings_used: int = mini(opening_savings, maxi(0, planned - available_income))
	var borrowed: int = mini(maxi(0, int(economy.credit_limit) - debt),
		maxi(0, planned - available_income - savings_used))
	var cash: int = available_income + savings_used + borrowed
	...
	return {
		...
		"budget_deficit": maxi(0, planned - available_income), "unmet_needs": planned - paid,
		"food_security": food_security,
		"living_standard": "destitute" if food_security < 800 else (
			"poor" if planned > available_income or planned > paid else "basic")
	}


static func validate_ledger(ledger: Dictionary) -> Array[String]:
	var errors: Array[String] = []
	if ledger.opening_savings + ledger.earned_income + ledger.external_income + ledger.borrowed != \
	...
	if ledger.earned_income + ledger.external_income != ledger.available_income:
		errors.append("Available income does not match wages and external support")
	for key: String in ["opening_savings", "closing_savings", "opening_debt", "closing_debt",
			...
			"planned_expenses", "paid_expenses", "rent_paid", "food_paid", "essentials_paid", "external_income"]:
		if ledger[key] < 0:
